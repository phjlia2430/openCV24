...
#2 사칙연산
img_plus = gray + 50       # y = x + 50 #gray가 입력이미지
...
pp=np.hstack((gray, img_plus, img_minus, img_multi, img_div, img_reverse)) # hstack은 높이(세로)가 같아야 함 #하나로 쭉 이어서 붙임
cv2.imshow('point processing',pp)

#3 opencv 함수
img_plus2 = cv2.add(gray, 50)         # y = x + 50
cv2.putText(img_plus2,"add(50)",(20,30),cv2.FONT_HERSHEY_PLAIN,1.5, 0,2) #결과값을 보기좋게 출력해주는 구문
...
img_addW1 = cv2.addWeighted(img, 0.5, img2, 0.5, 0) # 이미지 블렌딩(Image Blending)
img_addW2 = cv2.addWeighted(img, 0.8, img2, 0.2,0)
pp4=np.hstack((img_plus3, img_minus3, img_multi3, img_div3,img_addW1,img_addW2)) # hstack은 높이(세로)가 같아야 함
