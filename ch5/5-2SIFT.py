...
kp,des=sift.detectAndCompute(img,None)  # SIFT 특징점 검출과 특징 디스크립터(기술자) 계산을 한 번에 수행
# 2mask : 특징점 검출에 사용할 필터

print(len(kp))
print(kp[0].pt, kp[0].size, kp[0].octave, kp[0].angle)
print(des[0])
print(des[0].shape) #128

dst=cv2.drawKeypoints(img,kp, None, flags=cv2.DRAW_MATCHES_FLAGS_DEFAULT)   # 특징점을 그림
dst_rich=cv2.drawKeypoints(img,kp, None, flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS) #스케일에 따라 원 크기 다르고 앵글도 표현
